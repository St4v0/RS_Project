...
#define NB_BS_PIN 2
#define R_EXPONENT_WEIGHT 0.7//Exponent weight for right sensor
#define L_EXPONENT_WEIGHT 0.7//Exponent weight for left sensor
...
#define CALIBRATION_TIME 10000000 //calibration time in microseconds 
#define STEP_DOWN_LEFT 0.6
...
class BumpSensor_c {
  public:
    ...
    void initialise(){
      int i;
      for(i = 0; i < NB_BS_PIN; ++i) pinMode(bs_pin[i], INPUT);
      for(i = 0; i < NB_BS_PIN; ++i) SUM[i] = 0;
      ...
      while(done == false){
        ...
        while(read_end[0] == false || read_end[1] == false){
          for(int Which_Sensor = 0; Which_Sensor < NB_BS_PIN; ++Which_Sensor){
            Which_Sensor == 1 ? temp_elapsed_time = (micros() - start_time)*0.01*STEP_DOWN_LEFT : temp_elapsed_time = (micros() - start_time)*0.01;//take elapsed time         
        ...
        if(elapsed_cal_time > CALIBRATION_TIME){
          int i = 0;
           ...
           for(i = 0; i < NB_BS_PIN; ++i) SUM[i] = 0;
           for(i = 0; i < WINDOW_SIZE; ++i) Left_Readings[i] = 0;
           for(i = 0; i < WINDOW_SIZE; ++i) Right_Readings[i] = 0;
        }
      }
    }
      ...
      while(done == false){    
        for(int Which_Sensor = 0; Which_Sensor < NB_BS_PIN; ++Which_Sensor){//loop through the sensor array
          ...
          if(digitalRead(bs_pin[Which_Sensor]) == LOW && !read_end[Which_Sensor]){//if sensor is discharged and hasn't been previously read
            ...
            prior_AVERAGE[Which_Sensor] = AVERAGE[Which_Sensor];
...
//          Display_Lapsed_Time();//display elapsed time
   ...
   void Display_Lapsed_Calibration(){
          ...
          Serial.print(", ");
          Serial.println(min_average_time[0]);
          Serial.print(max_average_time[1]);
          Serial.print(", ");
...
//          Serial.print(", ");
          ...
          Serial.print(", R ");
          Serial.println(AVERAGE[1]*1);
    }
    
  private:
    float prev_elapsed_time[NB_BS_PIN] = { 0 };
    float prior_AVERAGE[NB_BS_PIN] = { 0 };
    int INDEX = 0;
    ...
    float max_average_time[NB_BS_PIN] = { 1500, 1500 }; //to store max discharge time duting Calibration
    float min_average_time[NB_BS_PIN] = { 0, 0}; //to store min discharge time during Calibration
    ...
    void PT_Charge(){
      int i = 0;
      for(i = 0; i < NB_BS_PIN; ++i) elapsed_time[i] = 0;//reset the dischqrge time to zero 
      for(i = 0; i < NB_BS_PIN; ++i) read_end[i] = false;//reset the read boolean to flase
      for(i = 0; i < NB_BS_PIN; ++i) pinMode(bs_pin[i], OUTPUT);//reset LED pins to OUTPUT mode
      for(i = 0; i < NB_BS_PIN; ++i) digitalWrite(bs_pin[i], HIGH);//Set the LED PINS to HIGH to charge capacitor
      delayMicroseconds(30);
      done = false;//reset done state to false
      for(i = 0; i < NB_BS_PIN; ++i) pinMode(bs_pin[i], INPUT); //set the LEDs to INPUT mode

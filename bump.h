...
//      Serial.print("\nBump sensor FINAL VALUES!: ");
  ...
  private:
    double AvgCal[NB_BS_PIN]= { 0 };
    const int bs_pin[NB_BS_PIN] = {LEFT_BUMP_SENSOR, RIGHT_BUMP_SENSOR};
    bool read_end[NB_BS_PIN];
    bool done;
    double SUM[NB_BS_PIN] = { 0 };
    float AVERAGE[NB_BS_PIN] = { 0 };
    unsigned long Right_Readings[WINDOW_SIZE] = { 0 };
    unsigned long Left_Readings[WINDOW_SIZE] = { 0 }; 
    ...
    void PT_Charge(){
      for(int k = 0; k < NB_BS_PIN; ++k) elapsed_time[k] = 0;
      for(int k = 0; k < NB_BS_PIN; ++k) read_end[k] = false;
      for(int i = 0; i < NB_BS_PIN; ++i) pinMode(bs_pin[i], OUTPUT);
      for(int j = 0; j < NB_BS_PIN; ++j) digitalWrite(bs_pin[j], HIGH);
